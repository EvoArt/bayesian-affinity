...
\lambda_{i} &~ \text{N}(0,\lambda_{\sigma})\\
\end{aligned}
$${#eq-BDglm}

where $\beta_{\sigma}$ and $\lambda_{\sigma}$ are to be chosen according to the specific of the system being analysed. 

# Discussion 

Here, I have shown how to construct a Bayesian dyadic GLM for the analysis of cooccurrence data. This builds on the work of Mainali et al. [-@affinity] as well as Veech [-@veech2013probabilistic] and Griffith et al. [-@griffith2016cooccur]. The identification of Fisher's noncentral hypergeometric distribution (or mathematically equivalent formulations) as the correct distribution for modelling cooccurrence led first to null model approaches to cooccurrence analysis [@griffith2016cooccur;@veech2013probabilistic], then to a useful cooccurrence metric [@affinity] and now to general model capable of analysing raw cooccurrence data as a response variable even when data points are not independent (which will generally be the case). 
